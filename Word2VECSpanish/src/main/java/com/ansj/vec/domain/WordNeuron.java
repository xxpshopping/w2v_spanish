...
public class WordNeuron extends Neuron {
  ...
  public WordNeuron(String name, double freq, int layerSize) {
    this.name = name;//词
    this.freq = freq;//词频
    this.syn0 = new double[layerSize];//词向量，layerSize是向量维度
    this.tempsyn0 = new double[layerSize];//词的辅助向量
    Random random = new Random();//随机数
    for (int i = 0; i < syn0.length; i++) {
      syn0[i] = (random.nextDouble() - 0.5) / layerSize;//随机初始化词向量
      tempsyn0[i] = (random.nextDouble() - 0.5) / layerSize;//随机初始化词的辅助向量
    }
  }
  ...
  public WordNeuron(String name, double freq, int category, int layerSize) {
    ...
    this.tempsyn0 = new double[layerSize];
    this.category = category;//类别？
    Random random = new Random();
    for (int i = 0; i < syn0.length; i++) {
    	syn0[i] = (random.nextDouble() - 0.5) / layerSize;
    	tempsyn0[i] = (random.nextDouble() - 0.5) / layerSize;
    }
  }
